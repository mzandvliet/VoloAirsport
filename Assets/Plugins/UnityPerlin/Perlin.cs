...
namespace UnityNoise {
    public class SmoothRandom {
        public static Vector3 GetVector3(float time, float speed) {
            float timeScaled = time*0.01F*speed;
            return new Vector3(Get().HybridMultifractal(timeScaled, 15.73F, 0.58F),
                               Get().HybridMultifractal(timeScaled, 63.94F, 0.58F),
                               Get().HybridMultifractal(timeScaled, 0.2F, 0.58F));
        }

        public static Vector3 GetSignedVector3(float time, float speed) {
            float timeScaled = time*0.01F*speed;
            return new Vector3(Get().HybridMultifractal(timeScaled, 15.73F, 0.58F) * 2f - 1f,
                               Get().HybridMultifractal(timeScaled, 63.94F, 0.58F) * 2f - 1f,
                               Get().HybridMultifractal(timeScaled, 0.2F, 0.58F) * 2f - 1f);
        }

        public static float Get(float time, float speed) {
            float timeScaled = time*0.01F*speed;
            return Get().HybridMultifractal(timeScaled*0.01F, 15.7F, 0.65F);
        }

        private static FractalNoise Get() {
            if (s_Noise == null)
                s_Noise = new FractalNoise(0, 1.27F, 2.04F, 8.36F);
            return s_Noise;
        }
    ...
    public class FractalNoise {
        public FractalNoise(int seed, float inH, float inLacunarity, float inOctaves)
            : this(seed, inH, inLacunarity, inOctaves, null) {}

        public FractalNoise(int seed, float inH, float inLacunarity, float inOctaves, Perlin noise) {
