...
namespace StringLeakTest
{
	public static partial class StringBuilderExtensions
	{
		...
		public static StringBuilder Concat( this StringBuilder string_builder, float float_val, uint decimal_places, uint pad_amount, char pad_char )
		{
			...
			{
			    var roundedValue = Math.Round(float_val, (int)decimal_places);
			    var integerPart = (int)Math.Truncate(roundedValue);
			    var decimalPart = Math.Abs(roundedValue - integerPart);
			    decimalPart *= Math.Pow(10, decimal_places);
			    var decimalPartInt = (int)Math.Truncate(decimalPart);

				// First part is easy, just cast to an integer
				string_builder.Concat(integerPart, pad_amount, pad_char, 10);
				// Decimal point
				string_builder.Append('.');
                // TODO Prepend zeroes if there are any
			    var trailingZero = ((int)decimal_places) - 1;
			    while (trailingZero > 0 && decimalPartInt < Math.Pow(10, trailingZero)) {
			        string_builder.Append('0');
			        trailingZero--;
			    }
                string_builder.Concat(decimalPartInt, 0, '0', 10);
			}
			return string_builder;
		}
