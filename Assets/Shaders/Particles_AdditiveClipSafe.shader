...
Shader "Particles/Additive (Softy)" {
Properties {
	_Color ("Main Color", Color) = (1,1,1,1)
	_MainTex ("Particle Texture", 2D) = "white" {}
	_InvFade ("Soft Particles Factor", Range(0.01,3.0)) = 1.0
	_ClipFadeDistance ("Clip Fade Distance", Range(0.01, 2.0)) = 1.0
}
	...
	SubShader {
		Pass {
			...
			v2f vert (appdata_t v)
			{
				...
				//float alpha = (-viewPos.z - _ProjectionParams.y)/_ClipFadeDistance;
				//alpha = min(alpha, 1);
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				float fade = saturate (_InvFade * (sceneZ-partZ)) * saturate(partZ * _ClipFadeDistance - 1.0);
				i.color.a *= fade;
				#endif
